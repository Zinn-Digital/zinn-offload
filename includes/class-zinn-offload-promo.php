 ...
 */
final class Zinn_Offload_Promo {
	 ...
	 */
	private const NAME = "\u{2068}Zinn® Media Offload\u{2069}";
	...
	public static function guide_url(): string {
		return 'https://zinndigital.com/kb/zinn-offload-plugin';
	}
	...
	public static function add_widget(): void {
		...
		wp_add_dashboard_widget(
			...
			sprintf( __( '%s — guides and services', 'zinn-offload' ), self::BRAND ),
	...
	public static function render_widget(): void {
		...
		<div class="zinn-promo">
			<?php if ( array() !== $installed ) : ?>
				<p><strong><?php echo esc_html__( 'User guides', 'zinn-offload' ); ?></strong></p>
				<ul>
	...
	public static function render_panel(): void {
		...
		<div class="zinn-promo card" style="max-width:52em;padding:0 1.5em 1em;margin-top:2em;">
			...
			$heading = sprintf(
				/* translators: %s: the Zinn Digital trade mark, which is not translated. */
				__( '%s — guides and services', 'zinn-offload' ),
				self::BRAND
			);
			?>
			...
			$guide_label = sprintf( __( 'Read the %s user guide', 'zinn-offload' ), self::NAME );
			?>
			<p>
			...
			<p>
				<a href="<?php echo esc_url( self::dismiss_url() ); ?>">
					<?php echo esc_html__( 'Hide this', 'zinn-offload' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	private static function render_offers(): void {
		...
		if ( ! self::is_zinn_hosted() ) {
			$offers[] = array(
				...
				'title' => __( 'Zinn® hosting for WordPress', 'zinn-offload' ),
				'body'  => __( 'LiteSpeed, daily backups, free migration and a control panel built for people who run more than one site.', 'zinn-offload' ),
			);
		}

		$offers[] = array(
			'url'   => 'https://zinndigital.com/marketplace',
			'title' => __( 'The Zinn® marketplace', 'zinn-offload' ),
			'body'  => __( 'Buy and sell established sites, domains and link placements, with the money held safely until both sides are happy.', 'zinn-offload' ),
		);
		...
		$offers[] = array(
			...
			'title' => __( 'Zinn Hub® — hire people who do this for a living', 'zinn-offload' ),
			'body'  => __( 'Our global freelance marketplace: browse over 10,000 ready-made services from writers, designers, developers and SEOs, or post a project and let them bid.', 'zinn-offload' ),
		);
